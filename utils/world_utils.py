...
def run_world(wav_name, world_name, out_path = '', inf0 = None, out_files_names = None):
    ...
    if out_files_names is None:
        ...
        f0_file_name = out_files_names['f0']
        sp_file_name = out_files_names['sp']
        ap_file_name = out_files_names['ap']
...
def run_world_by_reaper(wav_name, out_path = '', rpath = None, wpath = None, out_files_names = None):
    ...
    wrld_result = \
        run_world(wav_name, WORLD_PATH, out_path, inf0 = reaper_f0_bin_file, out_files_names=out_files_names)

    print("Results are : {0}, {1}, {2}".format(wrld_result[0], wrld_result[1], wrld_result[2]))

    return wrld_result

def try_search_previous_world_results(wav_name, out_path):
