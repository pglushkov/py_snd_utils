...
def cut_sig_into_chunks(signal, chunk_size, overlap_size = 0, pad_zeros = True):
    assert is_row(signal)
    in_len = signal.shape[1]
    if pad_zeros:
        if overlap_size > 0:
            num_steps = int( numpy.ceil(float(in_len) / overlap_size) )
            total_size = num_steps * overlap_size + (chunk_size - overlap_size)
            num_zeros = total_size - in_len
            tmp = numpy.hstack( (signal, numpy.zeros((1, num_zeros))) )
            out = numpy.zeros( (num_steps, chunk_size))
            for k in range(num_steps):
                out[k,:] = tmp[0, k * overlap_size : (k * overlap_size) + chunk_size]
            return out
        else:
        ...
        if overlap_size > 0:
            num_steps = int( numpy.ceil(float(in_len - chunk_size) / overlap_size) )
            out = numpy.zeros( (num_steps, chunk_size))
            for k in range(num_steps):
                out[k,:] = signal[0, k * overlap_size : (k * overlap_size) + chunk_size]
            return out
        else:
